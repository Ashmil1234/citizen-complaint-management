(() => {
  ...
  const departmentDefaults = {
    ...
    "Electricity": { sla: 5, icon: "1", reason: "Power-service interruption or public-lighting issue." },
  ...
  const seedCases = [
    {
      id: "HX-260825-1042",
      citizenName: "Sreedevi Nair",
      contact: "sreedevi.demo@example.com",
      district: "Ernakulam",
      locality: "Kochi Corporation, Ward 12",
      department: "Local Self Government",
      category: "Road or streetlight",
      description: "The streetlight near the bus stop has been off for three weeks. Older people and students are walking in the dark after 7 pm.",
      urgency: "priority",
      evidence: "ward12-streetlight.jpg",
      status: "In progress",
      assignedTo: "Ward service desk",
      createdAt: "2026-08-22T09:15:00+05:30",
      dueAt: "2026-08-29T09:15:00+05:30",
      actionNote: "Electrical inspection requested from the ward team.",
      history: [
        { at: "2026-08-22T09:15:00+05:30", label: "Case created" },
        { at: "2026-08-23T11:30:00+05:30", label: "Assigned to ward service desk" }
      ]
    },
    {
      id: "HX-260824-0881",
      citizenName: "Muneer K",
      contact: "",
      district: "Malappuram",
      locality: "Tirur Village Office",
      department: "Revenue",
      category: "Land or revenue record",
      description: "My land tax receipt and mutation request have been pending since the application was submitted last month.",
      urgency: "normal",
      evidence: "Application no. REV-11290",
      status: "New",
      assignedTo: "Revenue nodal desk",
      createdAt: "2026-08-24T15:00:00+05:30",
      dueAt: "2026-09-14T15:00:00+05:30",
      actionNote: "",
      history: [{ at: "2026-08-24T15:00:00+05:30", label: "Case created" }]
    },
    {
      id: "HX-260820-0714",
      citizenName: "Asha Thomas",
      contact: "asha.demo@example.com",
      district: "Kottayam",
      locality: "Kottayam Municipality, Ward 4",
      department: "Local Self Government",
      category: "Waste or drainage",
      description: "Open waste has accumulated beside the public drain near the market. The smell is affecting nearby homes and shops.",
      urgency: "urgent",
      evidence: "market-drain-video.mp4",
      status: "In progress",
      assignedTo: "Sanitation response team",
      createdAt: "2026-08-20T10:20:00+05:30",
      dueAt: "2026-08-23T10:20:00+05:30",
      actionNote: "Site visit completed; removal crew scheduled.",
      history: [
        { at: "2026-08-20T10:20:00+05:30", label: "Case created" },
        { at: "2026-08-21T09:00:00+05:30", label: "Site visit completed" }
      ]
    },
    {
      id: "HX-260817-0522",
      ...
      contact: "",
      district: "Thrissur",
      locality: "Government Higher Secondary School",
      department: "General Education",
      category: "Education service",
      description: "The disability certificate support desk has not replied to the student application even after two reminders.",
      urgency: "priority",
      evidence: "Student application EDU-8031",
      status: "Resolved",
      assignedTo: "School nodal officer",
      createdAt: "2026-08-17T12:00:00+05:30",
      dueAt: "2026-08-31T12:00:00+05:30",
      actionNote: "The school confirmed the application was forwarded and the family received an update.",
      resolutionNote: "Citizen was contacted and the pending application was forwarded to the district desk.",
      history: [
        { at: "2026-08-17T12:00:00+05:30", label: "Case created" },
        { at: "2026-08-19T14:40:00+05:30", label: "Resolved after school confirmation" }
      ]
    }
  ];
  ...
  // --- SUPABASE SETUP ---
  const SUPABASE_URL = "https://intqsddmvckijqwkgknu.supabase.co";
  const SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImludHFzZGRtdmNraWpxd2tna251Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODgzNDY2MzYsImV4cCI6MjEwMzkyMjYzNn0.X1NGp-F8jWiQesfrJcCto8kTwy7NWysHWdW6w5quqLE";
  const supabaseClient = window.supabase ? window.supabase.createClient(SUPABASE_URL, SUPABASE_ANON_KEY) : null;

  async function loadCases() {
    if (!supabaseClient) return clone(seedCases);
    try {
      const { data, error } = await supabaseClient
        ...
        .eq("id", "main")
        .single();
      if (error || !data) return clone(seedCases);
      return data.data;
    } catch (error) {
  ...
  async function persist() {
    try {
      ...
      if (supabaseClient) {
        await supabaseClient.from("cases_store").upsert({
          id: "main",
          data: cases,
  ...
  function getSlaState(item) {
    if (item.status === "Resolved" || item.status === "Rejected") {
      return { label: item.status, className: "resolved", days: null };
    }
  ...
  function renderQueue() {
    ...
    if (!visible.length) {
      list.innerHTML = `<div class="empty-queue"><h3>No matching cases</h3><p>Try another filter or restore the demo cases.</p></div>`;
      return;
    }
    ...
    list.innerHTML = visible.map((item) => {
      ...
      const firstLine = item.description.length > 90 ? `${item.description.slice(0, 90)}...` : item.description;
    ...
    $$('[data-open-case]', list).forEach((button) =>
  ...
  function renderInsights() {
    ...
    $("#department-chart").innerHTML = departments.length ? departments.slice(0, 7).map(([name, count]) => `
      <div class="bar-item">
    ...
    $("#outcome-chart").innerHTML = [["New", counts["New"]], ["In progress", counts["In progress"]], ["Resolved", counts["Resolved"]]].map(([label, count]) => `
      <div class="outcome-item"><strong>${count}</strong><span>${label}</span></div>`).join("");
    ...
    $("#root-cause-list").innerHTML = rootCauses.map(([label, count, detail]) => `
      <div class="root-cause"><strong>${escapeHtml(label)} <span class="nav-count">${count}</span></strong><p>${escapeHtml(detail)}</p></div>`).join("");
  ...
  function openCase(id) {
    ...
    detail.innerHTML = `
      <div class="drawer-head">
        <div>
          ...
          <h2 id="drawer-title">${escapeHtml(item.category)}</h2>
          <p>${escapeHtml(item.locality)}, ${escapeHtml(item.district)}</p>
        </div>
        <button class="drawer-close" id="close-drawer" type="button" aria-label="Close case review">Close</button>
      </div>
      <div class="detail-block">
        <div class="detail-grid">
          <div><span class="label-muted">Owner</span><p>${escapeHtml(item.department)}</p></div>
          <div><span class="label-muted">Priority</span><p><span class="priority-badge ${priority.className}">${priority.label}</span></p></div>
          <div><span class="label-muted">SLA status</span><p>${escapeHtml(sla.label)}</p></div>
          <div><span class="label-muted">Due date</span><p>${formatDate(item.dueAt)}</p></div>
        </div>
      </div>
      <div class="detail-block">
        <h3>Citizen description</h3>
        <p>${escapeHtml(item.description)}</p>
        <p class="helper-text">Evidence: ${escapeHtml(item.evidence || "Not supplied")}</p>
      </div>
      <div class="detail-block">
        <h3>Case timeline</h3>
        ${(item.history || []).map((event) => `<p><strong>${formatDate(event.at, true)}</strong> - ${escapeHtml(event.label)}</p>`).join("")}
      </div>
      <form class="detail-form" id="detail-form">
        ...
        <label for="detail-assignee">Assigned owner</label>
        <input id="detail-assignee" value="${escapeHtml(item.assignedTo || "")}" placeholder="Officer or service desk" />
        <label for="detail-action">Action taken / next action</label>
        <textarea id="detail-action" placeholder="Record a concrete action, handoff or request for evidence">${escapeHtml(item.actionNote || "")}</textarea>
        <label for="detail-resolution">Resolution evidence <span class="optional">Required for resolved cases</span></label>
        <textarea id="detail-resolution" placeholder="What changed, who confirmed it and when?">${escapeHtml(item.resolutionNote || "")}</textarea>
        <p class="form-error" id="detail-error" role="alert" hidden></p>
        <div class="drawer-actions">
          <button class="button button-primary" type="submit">Save case update</button>
          <button class="button button-quiet" id="cancel-drawer" type="button">Cancel</button>
        </div>
      </form>`;
    ...
    detail.setAttribute("aria-labelledby", "drawer-title");
    detail.focus({ preventScroll: true });

    $("#close-drawer").addEventListener("click", closeCase);
    $("#cancel-drawer").addEventListener("click", closeCase);

    $("#detail-form").addEventListener("submit", (event) => {
      event.preventDefault();
      const status = $("#detail-status").value;
      const resolutionNote = $("#detail-resolution").value.trim();
      const error = $("#detail-error");

      if (status === "Resolved" && resolutionNote.length < 12) {
        error.textContent = "Add a short resolution evidence note before marking this case resolved.";
        error.hidden = false;
        return;
      }

      item.status = status;
      item.assignedTo = $("#detail-assignee").value.trim() || "Unassigned";
      item.actionNote = $("#detail-action").value.trim();
      item.resolutionNote = resolutionNote;
      item.history = item.history || [];
      item.history.push({
        at: now().toISOString(),
        label: status === "Resolved" ? "Marked resolved with evidence" : `Status updated to ${status}`
      });
  ...
  function submitComplaint(event) {
    ...
    if (!form.checkValidity()) {
      error.textContent = "Please complete the required fields. The description should contain at least 20 characters.";
      error.hidden = false;
      form.reportValidity();
      return;
    }
    ...
    if (!values.consent) {
      error.textContent = "Please confirm the local-demo storage notice.";
      error.hidden = false;
      return;
    }
    error.hidden = true;

    const triage = analyse(values);
    const created = now();
    const id = `HX-${String(created.getFullYear()).slice(-2)}${String(created.getMonth() + 1).padStart(2, "0")}${String(created.getDate()).padStart(2, "0")}-${Math.floor(1000 + Math.random() * 9000)}`;

    const item = {
      id,
      citizenName: values.citizenName || "Anonymous citizen",
      contact: values.contact || "",
      district: values.district,
      locality: values.locality,
      department: triage.department,
      category: values.category,
      description: values.description.slice(0, MAX_DESCRIPTION),
      urgency: triage.urgency,
      evidence: values.evidence || "",
      status: "New",
      assignedTo: "Triage queue",
      createdAt: created.toISOString(),
      dueAt: addDays(created, triage.config.sla),
      actionNote: "Awaiting officer review.",
      history: [{ at: created.toISOString(), label: "Case created by citizen intake" }]
    };
    ...
    renderAll();
    form.reset();
    $("#char-count").textContent = "0 / 800";
    updateTriage();
    showToast(`${id} saved. It is ready for review.`);
    showView("officer");
    openCase(id);
  }
  ...
  function exportData() {
    ...
    link.href = url;
    link.download = `hack-x-civic-cases-${new Date().toISOString().slice(0, 10)}.json`;
    link.click();
  ...
  ["input", "change"].forEach((eventName) =>
    $("#complaint-form").addEventListener(eventName, updateTriage)
  );

  $("#description").addEventListener("input", (event) => {
    if (event.target.value.length > MAX_DESCRIPTION) {
      event.target.value = event.target.value.slice(0, MAX_DESCRIPTION);
    }
    $("#char-count").textContent = `${event.target.value.length} / ${MAX_DESCRIPTION}`;
  });

  $("#case-search").addEventListener("input", renderQueue);
  $("#case-filter").addEventListener("change", renderQueue);
  $("#export-top")?.addEventListener("click", exportData);
  $("#export-officer")?.addEventListener("click", exportData);
  $("#seed-demo")?.addEventListener("click", restoreDemo);

  document.addEventListener("keydown", (event) => {
    if (event.key === "Escape" && !$("#case-detail").hidden) closeCase();
  });
